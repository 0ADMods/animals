...
/**
 * Select active panel.
 * @param panelNumber                       Number of panel, which should get active state (integer) (for marking as active).
 * @param panelNames                        Array containing the names of all panels (for marking as inactive).
 * @param attributeValuePairsActive         Value of sprite attribute of button object when active.
 * @param attributeValuePairsInactive       Value of sprite attribute of button object when inactive.
 * @param objectToBeStyledName_toPrepend    If the objectToBeStyled differs from the panelObject (e.g. a button within the panel). 
 * @param objectToBeStyledName_toAppend     If the objectToBeStyled differs from the panelObject (e.g. a button within the panel). 
 * @param isAdjustingTabDividersRequired    If to adjust tab dividers (use only if those exist and adjusting not conflicts with other panels). 
 */
function selectPanel(panelNumber)
{
	var panelNames = arguments[1] || [ 'scorePanel', 'buildingsPanel', 'unitsPanel', 'resourcesPanel', 'marketPanel', 'miscPanel', 'chartPanel' ];
    var attributeValuePairsActive = arguments[2] || { "sprite": "ForegroundTab" };
    var attributeValuePairsInactive = arguments[3] || { "sprite": "BackgroundTab" };
    var objectToBeStyledName_toPrepend = arguments[4] != undefined ? arguments[4] : '';
    var objectToBeStyledName_toAppend = arguments[5] != undefined ? arguments[5] : 'Button';
    var isAdjustingTabDividersRequired = arguments[6] /*|| objectToBeStyledName_toAppend == 'Button' */ || true;
    print("\n" + '0: ' + arguments[0]);
    print("\n" + '1: ' + arguments[1]);
    print("\n" + '2: ' + arguments[2]);
    print("\n" + '3: ' + arguments[3]);
    ...
    {    
        ...
        var attributeValuePairs = attributeValuePairsActive;
        var isToBeHidden = false;
        var currentPanel_isAdjustingTabDividersRequired = isAdjustingTabDividersRequired;

	    if (i != panelNumber)
        {
            // => It's not the panel which shall be selected. => Hide the object and don't perform special action. 
            attributeValuePairs = attributeValuePairsInactive;
            isToBeHidden = true;
            currentPanel_isAdjustingTabDividersRequired = false;
        }

        // SHOW OR HIDE:
		var panel = Engine.GetGUIObjectByName(panelNames[i]);
        if (panel != undefined)
            panel.hidden = isToBeHidden;

        // STYLE IF ACTIVE TAB:
	    var objectToBeStyledName = objectToBeStyledName_toPrepend + panelNames[i] + objectToBeStyledName_toAppend; //<-- for debugging.
        print("\n" + 'Looking for sprite object (most often a button): ' + objectToBeStyledName);
		var objectToBeStyled = Engine.GetGUIObjectByName(objectToBeStyledName);
        if (objectToBeStyled != undefined && attributeValuePairs != undefined && typeof attributeValuePairs == 'object')
        {
            
            for (var key in attributeValuePairs) {

                print("\nAttribute: " + key + " \tValue: " + attributeValuePairs[key]);
                /*
                // Is the given sprite attribute value a background sprite? TODO Test if this makes sense. Not yet fully understood why it not works. (there should be a black border around the active sprite XML object but it is not.)
                var first2chars = spriteValue.substr(0,2).toLowerCase();
                if (first2chars == 'bg' || first2chars == 'bk')
                    objectToBeStyled.sprite_background = spriteValue;
                else
                */
                
		        objectToBeStyled[key] = attributeValuePairs[key];
            
                if (currentPanel_isAdjustingTabDividersRequired)
		            adjustTabDividers(objectToBeStyled.size);
        ...
        {
            // check if this panel was a complete failure: neither objectToBeStyled/button nor panel exist/found.
            if (panel == undefined) 
                print("\n" + 'Both panel and objectToBeStyled XML element not found. => nothing to do, therefore no panel can be selected. panel: ' 
                        + panel + '  objectToBeStyled: ' + objectToBeStyled);
            print("\n" + 'objectToBeStyled undefined: ' + objectToBeStyled + "\n");
        }
